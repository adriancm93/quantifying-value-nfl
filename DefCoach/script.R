...
sample<-readRDS(
  url("https://github.com/adriancm93/quantifying-value-nfl/blob/master/pbp_mut.RDS?raw=true")
)%>% 
...
main_start <- Sys.time()
# index
setwd("~/GitHub/quantifying-value-nfl/DefCoach")
rm(.Random.seed, envir=globalenv())
start_time <- Sys.time();indx <- sampler(sample, "def_coach", reps = 150);end_time <- Sys.time();(end_time - start_time)
# resample
...
boot <- dplyr::bind_rows(lst);def_coach<-data.frame(coef = boot$def_coach..Intercept.,effect = 'DefCoach')
print(paste('N samples =',length(def_coach$coef)))
rm(boot)
main_end <- Sys.time();print(main_end - main_start)

ggplot(def_coach,aes(x=coef,fill=effect))+
