...
filename <- paste0('results/',stringr::str_replace_all(time,":",""),'_100.RDS')
...
boot <- dplyr::bind_rows(lst);team<-data.frame(coef = boot$POSC_rank..Intercept.,effect = 'OffensiveSupportingCast')
ggplot(team,aes(x=coef,fill=effect))+
  geom_density(alpha=.4)+theme_bw()+theme(legend.position = 'top')
print(paste('N samples =',length(team$coef)))
rm(boot)
