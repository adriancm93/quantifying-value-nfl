...
start_time <- Sys.time(); mixed_model<-sample %>% 
  lmer(formula=
         ...
         wp + 
         (1|passer_player_id) +
         (1|passer_player_id:POSC_rank)+
         (1|passer_player_id:PDSC_rank)
       ,
...
boot <- dplyr::bind_rows(lst);qb<-data.frame(coef = boot$passer_player_id..Intercept.,effect = 'QB')
ggplot(qb,aes(x=coef,fill=effect))+
  geom_density(alpha=.4)+theme_bw()+theme(legend.position = 'top')
print(paste('N samples =',length(qb$coef)))
rm(boot)
