...
filename <- paste0('results/',stringr::str_replace_all(time,":",""),'.RDS')
...
boot <- dplyr::bind_rows(lst);pos_coach<-data.frame(coef = boot$pos_coach..Intercept.,effect = 'Coach')
print(paste('N samples =',length(pos_coach$coef)))
rm(boot)
main_end <- Sys.time();print(main_end - main_start)

ggplot(pos_coach,aes(x=coef,fill=effect))+
